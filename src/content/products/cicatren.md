---
sku: "cicatren-o3"
wooId: 136
line: "dispositivi-medici"
class: "IIb"
deviceType: "dispositivo-medico"
manufacturer: "Biostamina KFT"
registration: "Dispositivo medico CE · Registrato al Ministero della Salute · Venduto in farmacia · Made in Italy"
category: "cicatrizzazione"
format: "Lipogel cicatrizzante"
price: 37.80
priceList: 60.00
inStock: true
featured: true
order: 2
draft: false
heroImage: ../../assets/products/cicatren-01.jpg
images:
  - ../../assets/products/cicatren-01.jpg
inci:
  - "PRORIGENERA Hypericum Perforatum Oil"
  - "Prunus Amygdalus Dulcis Ozonated Oil"
  - "Calendula Officinalis Oil"
  - "Hydrogenated Castor Oil"
  - "Citrus Medica Limonum Peel Oil"
  - "Lavandula Angustifolia Oil"
  - "Citrus Grandis Peel Oil"
  - "Camomilla Recutita Oil"
  - "Geranium Maculatum Oil"
  - "Silica"
  - "Echinacea Angustifolia Oil"
  - "Manganese Gluconate"
  - "Tocopheryl Acetate"
it:
  name: "Bio RIGENERA® O3 Cicatren — Lipogel Cicatrizzante"
  description: |
    Bio RIGENERA® O3 Cicatren lipogel topico. Formulazione composta da oli essenziali arricchiti con ozono, indicato per ferite ed ustioni della pelle, lesioni post-chirurgiche e piaghe da decubito; accelera il processo di guarigione delle lesioni cutanee croniche, ferite acute e traumatiche. Forma una pellicola protettiva sulla lesione, producendo un effetto barriera che contribuisce ad alleviare il dolore e favorisce la guarigione.
  indications:
    - "Ferite da taglio"
    - "Ustioni della pelle"
    - "Ferite chirurgiche e traumatiche"
    - "Piaghe da decubito ulcerose, acute e croniche"
    - "Deiscenza delle ferite"
    - "Ulcere del piede diabetico"
    - "Ulcere da stasi venosa"
  properties: |
    Lipogel protettivo e cicatrizzante composto da oli essenziali arricchiti con ozono. Associando i differenti principi attivi presenti negli oli essenziali con componenti antisettici, rigeneranti e con attività citoprotettiva, si espleta una azione cicatrizzante, importante nel favorire la sana guarigione delle ferite, limitando al contempo il rischio di infezioni batteriche. L'uso topico consente di ottimizzare l'attività in loco.
  phyto: |
    Le proprietà dei semi di pompelmo e dell'ozono svolgono un'azione antibatterica e contribuiscono ad una veloce e corretta rigenerazione del tessuto epiteliale.
  usage: |
    Va applicato sulla parte da trattare, una o due volte al giorno fino alla completa guarigione.
  warnings: |
    Non utilizzare in caso di ipersensibilità o allergia note verso uno o più componenti. Evitare il contatto con gli occhi; in caso di contatto risciacquare con abbondante acqua fresca. Se dovessero verificarsi intolleranze o altri effetti avversi, sospendere l'uso ed eventualmente consultare il medico. Tenere fuori dalla portata dei bambini. La data di scadenza si riferisce al prodotto integro e correttamente conservato in luogo asciutto. Si consiglia di usare il prodotto entro 12 mesi dall'apertura. Non sono conosciute interazioni. Per l'uso contemporaneo a trattamenti farmacologici topici, consultare il medico. Eventuali variazioni del prodotto quali consistenza e colore, sono da attribuire alla variabilità del raccolto di ingredienti vegetali. Evitare l'esposizione diretta ai raggi solari.
en:
  name: "Bio RIGENERA® O3 Cicatren — Cicatrizing Lipogel"
  description: |
    Bio RIGENERA® O3 Cicatren topical lipogel. Formulation composed of essential oils enriched with ozone, indicated for wounds and skin burns, post-surgical lesions and decubitus (pressure sores); it accelerates the healing process of chronic skin lesions and of acute and traumatic wounds. Forms a protective film on the lesion, producing a barrier effect that helps to relieve pain and promotes healing.
  indications:
    - "Cutting wounds"
    - "Skin burns"
    - "Surgical and traumatic wounds"
    - "Acute and chronic decubitus ulcers"
    - "Wound dehiscence"
    - "Diabetic foot ulcers"
    - "Venous stasis ulcers"
  properties: |
    Protective and cicatrizing topical lipogel made with ozone enriched essential oils. By associating the various active ingredients present in essential oils with antiseptic, regenerative and cytoprotective activity, a cicatrizing action is performed, important in promoting the healthy healing of wounds while limiting the risk of bacterial infections. Topical use allows you to optimize on-site activity.
  phyto: |
    The properties of grapefruit seeds and ozone have an antibacterial effect and contribute to quick and correct epithelial tissue regeneration.
  usage: |
    Apply to the area to be treated one or two times a day, until the entire healing.
  warnings: |
    Do not use in case of confirmed or suspected hypersensitivity or allergy to one or more components. Avoid contact with eyes; in case of contact rinse with fresh water. If you experience allergies or other adverse effects, discontinue to use it and consult your doctor. Keep out of reach of children. The expiry date refers to the product intact and properly stored in a dry place. It is recommended to use the product within 12 months from opening. No reported side effects. For the simultaneous use of topical medication treatments, consult your doctor. Any variations in the product, such as consistency and colour, are to be attributed to the variability of the crop of vegetable ingredients. Avoid exposure to direct sunlight.
---

<!--
  Copy 100% verbatim dai bugiardini ufficiali (Rev. 06 12/2020).
  INCI con prefisso "PRORIGENERA" come da bugiardino. Classe IIb.
-->
